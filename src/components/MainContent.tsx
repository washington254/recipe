...
const MainContent = () => {
  return (
    <div className="w-full  flex flex-col gap-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-6 w-full">
        ...
        <div className="w-full md:max-w-md relative">
          <input 
            ...
            className="w-full py-3.5 pl-6 pr-12 rounded-xl border border-amber-300 bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-amber-400 text-gray-700 font-medium placeholder-gray-400"
          />
        ...
        <div className="flex items-center gap-5 shrink-0 self-end md:self-auto">
          ...
          {/* Premium Button */}
          <button className="bg-[#ffbf40] text-gray-900 font-semibold py-3 px-6 rounded-xl flex items-center gap-2 hover:bg-[#fca311] transition-transform hover:scale-[1.02] shadow-sm">
            Premium
      ...
      {/* Hero / Title Section */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mt-2 gap-8 md:gap-0">
        <h1 className="text-4xl md:text-5xl lg:text-[3.5rem] font-serif font-bold text-[#1e2329] leading-tight tracking-tight">
          Learn, Cook, &<br className="hidden md:block" /> Eat your food
        </h1>
        
        {/* Stats */}
        <div className="flex items-center gap-10 md:gap-14 mr-4">
      ...
      <div className="flex items-center gap-4 overflow-x-auto pb-4 pt-2 hide-scrollbar">
      ...
      {/* Recipe Cards Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 gap-y-24 mt-20 md:mb-10 w-full place-items-center">
        {recipes.map((recipe, index) => (
          <div key={index} className="relative w-full max-w-[280px] bg-white rounded-[2rem] shadow-[0_4px_25px_rgba(0,0,0,0.03)] flex flex-col pt-24 pb-2 px-1 border border-gray-100/50 hover:shadow-[0_8px_30px_rgba(0,0,0,0.06)] transition-all duration-300">
            
            {/* Overlapping Image */}
            <div className="absolute -top-[80px] left-1/2 -translate-x-1/2 w-[160px] h-[160px] rounded-full overflow-hidden shadow-lg border-4 border-white bg-white z-10 transition-transform duration-300 hover:scale-[1.03]">
              <img src={recipe.img} alt={recipe.title} className="w-full h-full object-cover" />
            </div>

            {/* Card Content Wrapper */}
            <div className="flex flex-col items-center px-4">
              <h3 className="font-bold text-gray-900 text-[17px] mb-2 text-center leading-snug h-12 flex items-center">{recipe.title}</h3>
              
              {/* Stars & Reviews */}
              <div className="flex items-center text-[#ffb703] text-sm mb-5 gap-[2px]">
